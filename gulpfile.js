...
gulp.task('xml', () => {
  return gulp.src([ src_assets_folder + 'xml/**/*.xml' ], { since: gulp.lastRun('xml') })
    .pipe(gulp.dest(dist_assets_folder + 'xml'))
    .pipe(browserSync.stream());
});
...
gulp.task('build', gulp.series('clear', 'html', 'sass', 'js', 'xml', 'images', 'vendor'));
...
gulp.task('watch', () => {
  ...
  gulp.watch(watchImages, gulp.series('xml')).on('change', browserSync.reload);
